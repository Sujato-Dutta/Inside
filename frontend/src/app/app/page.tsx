...
function ChartPanel({ id, title, subtitle, data, bars, query, onAsk, percent = true, reference = false, empty, legend = false }: {
  id?: string;
  ...
  return <section id={id} className="min-w-0 scroll-mt-32 rounded-2xl border border-[#E3DED4] bg-white p-5 shadow-sm">
...
export default function AppHomePage() {
  const { students, activeFiles, readinessScore, sessionDeleted, setPrefilledQuery, loadSampleDataset } = useSessionData();
  ...
  const strongest = stageRows.length > 1 ? [...stageRows].sort((a, b) => b["At / above"] - a["At / above"])[0] : undefined;
  const weakest = stageRows.length > 1 ? [...stageRows].sort((a, b) => a["At / above"] - b["At / above"])[0] : undefined;
  const onlyStage = stageRows.length === 1 ? stageRows[0] : undefined;
  const subjectRows = [
  ...
  const watchlist: Insight[] = [
    ...
    strongest ? { label: "Green strength · Key Stage", title: `${strongest.label} is the strongest measured stage`, detail: `${strongest["At / above"]}% of ${strongest.count} pupils are at or above expected.`, tone: "green", query: `Show current ${strongest.label} attainment against the 75% reference.` } : onlyStage ? { label: "Stage coverage", title: `${onlyStage.label} is the only measured stage`, detail: "Cross-stage comparison needs at least one more populated stage in the current dataset.", tone: "orange", query: `Show current ${onlyStage.label} attainment against the 75% reference.` } : { label: "Green strength · Evidence", title: "No Key Stage comparison available", detail: "Year group data is needed to identify a measured strength.", tone: "green", query: "Verify data quality and mapped year groups." },
  ];
  ...
  const evidenceTarget = (item: Insight) => item.label === "Data quality" || item.label.includes("Data coverage") ? "/app/data" : item.label.includes("Attendance") || item.label.includes("Red flag · Intervention") ? "#attendance-evidence" : item.label.includes("Inclusion") ? "#inclusion-evidence" : item.label.includes("Emirati") ? "#emirati-evidence" : item.label.includes("Cohort gap") ? "#subject-evidence" : "#stage-evidence";

  return <div className="space-y-6 pb-16">
    <section className="flex flex-col gap-4 rounded-3xl border border-orange-200 bg-orange-50/70 p-6 shadow-sm sm:p-7 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-[11px] font-extrabold uppercase tracking-[0.16em] text-orange-600">Executive evidence cockpit</p><h1 className="mt-2 text-3xl font-extrabold tracking-tight">Whole-school inspection readiness</h1><p className="mt-2 text-sm text-[#5C5852]">A visual summary of {total} current pupil records from {activeFiles.length} source file{activeFiles.length === 1 ? "" : "s"}.</p></div><div className="rounded-2xl border border-emerald-200 bg-white px-4 py-3 text-xs font-bold text-emerald-800"><Lock className="mr-2 inline h-4 w-4" />Browser memory active | purge in Data Hub</div></section>
    ...
    <section><div className="mb-4"><p className="text-[11px] font-extrabold uppercase tracking-wider text-orange-600">Current evidence</p><h2 className="mt-1 text-xl font-extrabold">Performance at a glance</h2><p className="mt-1 text-xs text-[#5C5852]">All charts update when the active dataset changes. The blue line is a school-defined 75% comparison reference, not an official UAE inspection threshold.</p></div><div className="grid gap-4 lg:grid-cols-2">
      <ChartPanel id="stage-evidence" title="Attainment by Key Stage" subtitle="At or above expected versus below expected, by phase." data={stageRows} bars={[{ key: "At / above", color: "#F97316", stackId: "stage" }, { key: "Below", color: "#DDD7CC", stackId: "stage" }]} reference legend query="Show current attainment by Key Stage against the 75% reference." onAsk={ask} />
      <ChartPanel id="subject-evidence" title="Core subject attainment" subtitle="Share of pupils at or above Grade 5 in each core subject." data={subjectRows} bars={[{ key: "At / above", color: "#F97316" }]} reference query="Compare whole-school attainment across English, Math, and Science against the 75% reference." onAsk={ask} />
      <ChartPanel id="attendance-evidence" title="Attendance profile" subtitle="Pupils grouped by their current attendance percentage." data={attendanceMix} bars={[{ key: "Share", color: "#E9A246" }]} query="Show current attendance groups and persistent absence." onAsk={ask} />
      ...
      <ChartPanel id="inclusion-evidence" title="Inclusion progress" subtitle="Average core grade change from Term 1 to Term 2." data={progressRows} bars={[{ key: "Grade change", color: "#279879" }]} percent={false} empty="Two-term marks for SEND pupils and peers are needed for this comparison." query="Compare observed SEND and non-SEND progress." onAsk={ask} />
      <ChartPanel id="emirati-evidence" title="Emirati cohort attainment" subtitle="Current attainment of Emirati pupils and other pupils." data={nationalRows} bars={[{ key: "Attainment", color: "#3E80BC" }]} reference empty="Emirati status and a comparison group are needed in the current data." query="Compare Emirati student attainment with the whole-school cohort." onAsk={ask} />
    </div></section>
    <section className="overflow-hidden rounded-2xl border border-[#E3DED4] bg-white shadow-sm"><div className="border-b border-[#E3DED4] bg-[#F9F6F0] px-5 py-4"><span className="inline-flex rounded-t-xl border-x border-t border-orange-200 bg-white px-4 py-2 text-sm font-extrabold text-orange-700">Important Insights</span><p className="mt-2 text-xs text-[#5C5852]">The most relevant findings from the loaded data, with a path to the supporting view.</p></div><ul className="divide-y divide-[#EEE9E1]">{insights.map((item) => <li key={item.label + item.title} className="flex flex-wrap gap-3 px-5 py-4"><span className={`mt-2 h-2.5 w-2.5 shrink-0 rounded-full ${toneClass(item.tone)}`} /><div className="min-w-0 flex-1"><p className="text-[10px] font-extrabold uppercase tracking-wider text-[#817B71]">{item.label}</p><p className="mt-1 text-sm font-extrabold">{item.title}</p><p className="mt-1 text-xs leading-relaxed text-[#5C5852]">{item.detail}</p></div><div className="flex items-center gap-2"><Link href={evidenceTarget(item)} className="rounded-lg px-2 py-1 text-[11px] font-extrabold text-[#5C5852] hover:bg-orange-50 hover:text-orange-700">View evidence</Link><button onClick={() => ask(item.query)} className="rounded-lg px-2 py-1 text-[11px] font-extrabold text-orange-700 hover:bg-orange-50">Ask Inside <ArrowRight className="inline h-3.5 w-3.5" /></button></div></li>)}</ul></section>
